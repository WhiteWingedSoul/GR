...
public class GooglePlayWrapper {
	...
	private List<GooglePlayElement> parseAppList(String httpBody) throws IOException {
		...
		for (Element card : cards) {
			...
			String elementDescription = card.select("div.description").get(0).text();

//			Document document = Jsoup.connect("https://play.google.com" +elementDetailsUrl).get();
...
//			Element description = document.select(".details-section").get(0);
//			String desc = description.select(".full-text").get(0).html();
//			Log.d("LOG","desc: "+desc);
//			String numberOfPage = document.select("span[numberOfPages]").html();
